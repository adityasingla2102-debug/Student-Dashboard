document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupBookFilters() {
        var buttons = document.querySelectorAll(".books-filter .filter-tab");
        var items = document.querySelectorAll(".book-card");
        var mapping = {
            "All Books": "all",
            "Physics": "physics",
            "Chemistry": "chemistry",
        ...
        function applyFilter(value, activeButton) {
            ...
            for (var j = 0; j < items.length; j++) {
                var matches = value === "all" || items[j].getAttribute("data-category") === value;
                items[j].style.display = matches ? "block" : "none";
            }
        }
    }

    function setupBookSelection() {
